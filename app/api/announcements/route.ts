...
export async function GET(req: NextRequest) {
  ...
  const docs = await Announcement.find(filter).sort({ created_at: -1 }).limit(10).lean();
...
// POST /api/announcements — owner/developer only
export async function POST(req: NextRequest) {
  ...
  const body = await req.json() as { text: string; siteId?: string; is_all_sites?: boolean };
